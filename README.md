...
### AI Chat with File Upload (JavaScript)
```javascript
const formData = new FormData();
formData.append('file', imageFile); // or PDF file
formData.append('prompt', 'What do you see in this image?');
formData.append('username', 'player1');
formData.append('room', 'gaming');
...
const result = await response.json();
console.log(result.aiMessage.message); // AI analysis
console.log(result.fileInfo.url); // File URL
```
